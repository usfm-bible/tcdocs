...
Description:: xref:note:footnote/f.adoc[Footnote] - Origin reference.
Syntax::
* *USFM:* `+\fr_{cv-reference}+`
* *USX:* `+<char style="fr">{cv-reference}</char>+`
`cv-reference`::: Chapter + verse origin reference. The separator character between chapter and verse is specific to the project text, but is commonly a colon `:` or period `.`. Trailing punctuation after the reference is optional.
// tag::spec[]
Added:: 1.0
// end::spec[]
