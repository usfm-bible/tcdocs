...
* *USX:* ``++<para style="++``#__style__#``++" ++``#__@vid @h__#``++>++``#__content__#``++</para>++``
...
#h#::: Running header text. _Only used when a paragraph is a part of a fragment of scripture_ (e.g. a scripture module), and the fragment includes a xref:ms:vid.adoc[vid] (current reference identifier) milestone.
