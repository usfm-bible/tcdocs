...
Normally USX represents USFM xref:para:index.adoc[paragraph] and xref:char:index.adoc[character] types with corresponding <para> and <char> tags. For tables, the structures are different. In *USX* the sequence of `\tr` rows is composed within a `+<table>...</table>+` container.
