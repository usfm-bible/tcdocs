...
* Add explicit xref:ROOT:syntax-usv.adoc[Unicode USV representation] for USFM (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25004%20Explicit%20Unicode.md[U25004^]).
* An optional `list` element in USX, and two pairs of xref:ms:index.adoc[milestones] in USFM to represent the start and end of a xref:ms:list.adoc[list] or xref:ms:table.adoc[table] (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25003%20Lists%20and%20Tables.md[U25003^]).
* Use a `++*++` (wildcard) to attribute in markers.ext (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2024/U24002%20Textual%20Alternatives.md[U24002^]). E.g. `++a-*++` to allow anything with an `++a-++` prefix.
* Generalised node initial attributes (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25001%20Attributes.md[U25001^]). E.g. `\ip |aid="author"| The author is unknown`.
* Anchors and inline referencing (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25002%20Anchors.md[U25002^])
* New xref:ROOT:extensions.adoc[markers.ext] xref:ROOT:extensions.adoc#ext-category[category] `standalone` - A bare milestone with no attibutes or delimeter.

=== Revisions

* xref:cv:v.adoc[v] is not allowed in xref:para:index.adoc[paragraphs] of xref:ROOT:extensions.adoc#ext-category[category] `otherpara` or `sectionpara`

== 3.1.2

=== Additions

* Add #__@lang__# attribute to xref:char:index.adoc[character] types xref:char:features/tl.adoc[tl] and xref:char:features/wl.adoc[wl]
* Add character type xref:char:features/pl.adoc[pl] (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25009%20Pl%20Marker.md[U25009^])
* Add character type xref:char:features/ta.adoc[ta] (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2024/U24002%20Textual%20Alternatives.md[U24002^])
* Add character type xref:char:features/wl.adoc[wl]
* Add new marker xref:ROOT:extensions.adoc#ext-category[categories]:
** `cell` (tc1..12, tcc1..12, tcr1..12, th1..12, thc1..12, thr1..12)
** `attribute` (xref:cv:cp.adoc[cp], xref:cv:vp.adoc[vp], xref:cv:ca.adoc[ca], xref:cv:va.adoc[va], xref:doc:usfm.adoc[usfm], xref:cat:cat.adoc[cat]) indicates that the contents are an attribute in USX.
* Add xref:ms:vid.adoc[vid] milestone to specify current reference (TC proposal: https://github.com/usfm-bible/tcdocs/blob/main/proposals/2025/U25008%20vid.md[U25008^])

== 3.1.1

=== Additions

* Add xref:para:index.adoc[paragraph] type xref:para:introductions/ipc.adoc[ipc] to correspond to xref:para:paragraphs/pc.adoc[pc].
...
* Allow xref:para:index.adoc[paragraph] type xref:para:paragraphs/lit.adoc[lit] in book xref:doc:index.adoc#doc-book-intro[introductions].
* xref:doc:books.adoc[Book codes] must contain at least one letter.
* xref:ROOT:extensions.adoc[Markers.ext] #_\category_# may contain multiple entries.
...
USFM (Unified Scripture Format Markup) 3.1 primarily represents an effort to resolve ambiguities, and to standardize a single xref:doc:index.adoc[content model] which can be equally expressed in backslash-based `\` format markers (USFM-FM), XML (USFM-XML), or JSON (USFM-JSON). The common abbreviations for these expressions are *USFM*, *USX*, and *USJ*.
...
Previous release notes (3.0 and earlier) are expressed only in USFM specific terms.
...
* xref:doc:index.adoc#doc-book-headers[BookHeaders] include xref:para:index.adoc[paragraph] type xref:para:identification/sts.adoc[sts].
* Distinguish xref:doc:index.adoc#doc-book-chapter-content[Section], xref:doc:index.adoc#doc-book-chapter-content[Verse], and Other xref:doc:index.adoc#doc-book-chapter-content[paragraph] types in the grammar (internal).
* The xref:para:index.adoc[paragraph] #_@vid_# attribute has an extended reference syntax.
* The xref:para:index.adoc[paragraph] type xref:para:paragraphs/mi.adoc[mi#] supports 3 indent levels.
...
** Note specific xref:char:notes/index.adoc[character] elements (i.e. `f`... xref:char:notes/footnote/fr.adoc[fr], xref:char:notes/footnote/fq.adoc[fq], xref:char:notes/footnote/ft.adoc[ft], or `x`... xref:char:notes/crossref/xo.adoc[xo], xref:char:notes/crossref/xt.adoc[xt] etc.) are considered note _structural elements_. Only these markers continue to support implicit closure.
** Note content must occur within a note structural element. There should be no unmarked note content occuring only within the xref:note:index.adoc[note container] itself.
