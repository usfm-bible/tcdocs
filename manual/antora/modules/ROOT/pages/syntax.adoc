...
Some markers or styles include a numeric variable, which is represented in this documentation by a hash character `+#+`. The number indicates:
...
*marker = marker1* — An _unnumbered_ version of a marker or style should only be used when one level of this marker exists within the text (only). _Numbered_ markers should always be used when more than one level of the marker exists within the text.

Numbered markers are _required_ in xref:char:tables/index.adoc[character types for tables] (table cells).
