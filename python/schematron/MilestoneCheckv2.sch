<schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern name="Qt Milestone checks">
        <rule context="ms[@style='qt-s']">
            <let name="nextQt" value="following::ms[@style='qt-s' or @style='qt-e'][1]" />
            <assert test="not(following::ms[matches(@style, 'qt[1-5]-s')])">qt-s should not be mixed with numbered qt styles</assert>
    ...
    <pattern name="QtN Milestone checks">
        <rule context="ms[matches(@style, 'qt[1-5]\-s')]">
            <let name="nextQt" value="following::ms[matches(@style, 'qt[1-5]\-[se]')][1]" />
            ...
            <assert test="($change gt -2) and ($change lt 2)">Levels should only change by 1</assert>
