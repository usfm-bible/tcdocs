...
class DebugParserGenerator(rply.ParserGenerator):
    def production(self, rule, precedence=None):
        ...
        def inner(func):
            print(f"Producing {production_name}")
            def debugproc(s, p):
                r = func(s, p)
                print(f"{production_name}(f{syms}): {p} -> {r}")
                return r
            self.productions.append((production_name, syms, debugproc, precedence))
            return debugproc
        return inner
...
@pg.production('particle : annotated-primary QMARK RSHIFT annotation-element')
def particle_maybe(s, p):
    return Node('SEQ', None, [Node('MAYBE', None, [p[0]]), p[3]])

@pg.production('particle : annotated-primary STAR RSHIFT annotation-element')
def particle_any(s, p):
    return Node('SEQ', None, [Node('ANY', None, [p[0]]), p[3]])

@pg.production('particle : annotated-primary PLUS RSHIFT annotation-element')
def particle_some(s, p):
    return Node('SEQ', None, [Node('SOME', None, [p[0]]), p[3]])

@pg.production('particle : annotated-primary RSHIFT annotation-element')
