...
class Sequence(DiagramMultiContainer):
    def __init__(self, *items, **kw):
        DiagramMultiContainer.__init__(self, 'g', items, **kw)
...
class Choice(DiagramMultiContainer):
    def __init__(self, default, *items, **kw):
        DiagramMultiContainer.__init__(self, 'g', items, **kw)
    ...
    def format(self, x, y, width):
        ...
        for i,ni,item in doubleenumerate(above):
            ...
            item.format(x + AR * 2, y - distanceFromY,
                    innerWidth if (noclose & 1) == 0 else 0).addTo(self)
            if (noclose & 1) == 0:
        ...
        self.items[self.default].format(x + AR * 2, y, 
                innerWidth if (noclose & 2) == 0 else 0).addTo(self)
        if (noclose & 2) == 0:
        ...
        for i, item in enumerate(below):
            ...
            item.format(x + AR * 2, y + distanceFromY,
                    innerWidth if (noclose & 4) == 0 else 0).addTo(self)
            if (noclose & 4) == 0:
...
class HorizontalChoice(DiagramMultiContainer):
    ...
    def format(self, x, y, width):
        # Hook up the two sides if self is narrower than its stated width.
        leftGap, rightGap = determineGaps(width, self.width)
        noclose = getattr(self, 'noclose', 0) != 0
        Path(x, y).h(leftGap).addTo(self)
        Path(x + leftGap + self.width, y + self.height).h(rightGap).addTo(self)
        x += leftGap
        ...
        for [i, item] in enumerate(self.items):
            ...
            # item
            itemWidth = item.width + (20 if item.needsSpace and not noclose else 0)
